...
- `required_with_all()`
- `Required_with_all_rule(array|string $dependetFields, array $dependentValue = [], array $values = [])`

Validates the input only if all of the dependent field is not empty.
...
$v->getValidator()->required_with_all(
	$dependent_fields, $dependent_values, $dependent_post_data
)
